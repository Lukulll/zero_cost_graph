...
from pruners.p_utils import *
from pruners import measures
...
def find_measures_arrays(net_orig, dname, device, measure_names=None, loss_fn=F.cross_entropy):
    ...
    inputs = get_some_graph_data(dname = dname, device=device)
        ...
        try:
            for measure_name in measure_names:
                if measure_name not in measure_values:
                    val = measures.calc_measure(measure_name, net_orig, device, inputs, loss_fn=loss_fn, split_data=ds)
...
def find_measures(net_orig,                  # neural network
                  dname,                     # name of the graph dataset
    ...
    if measures_arr is None:
        measures_arr = find_measures_arrays(net_orig, dname, device, measure_names=measure_names, loss_fn=loss_fn)
